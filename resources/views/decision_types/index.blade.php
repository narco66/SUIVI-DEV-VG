            ...
            <div class="h-100 rounded-4 border bg-primary bg-opacity-10 p-4 shadow-sm">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div>
                        ...
                        <h2 class="h3 fw-bold text-dark mb-1">Types de Décision</h2>
                        <p class="text-muted mb-2">Configurez les types utilisés pour classifier les décisions dans tout le système.</p>
                        <span class="badge bg-white text-primary border fw-semibold px-3 py-2">
                            {{ $types->total() }} type(s) enregistré(s)
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="h-100 w-100 d-flex flex-column justify-content-between rounded-4 border bg-white p-4 shadow-sm">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-2">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Types de décision</li>
                    </ol>
                </nav>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="bg-light text-muted small text-uppercase">
                    <tr>
                        <th class="ps-4 py-3">Nom du Type</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="ps-4 fw-medium text-dark">{{ $type->name }}</td>
                        <td><span class="badge bg-secondary-subtle text-secondary">{{ $type->code ?: 'N/A' }}</span></td>
                        <td class="text-muted small">{{ \Illuminate\Support\Str::limit($type->description, 60) ?: '-' }}</td>
                        <td class="pe-4 text-end">
                            <div class="btn-group">
                                <button type="button" class="btn btn-sm btn-light border" data-bs-toggle="modal" data-bs-target="#editModal{{ $type->id }}" title="Modifier">
                                    <i class="bi bi-pencil text-primary"></i>
                                </button>
                                <form action="{{ route('decision-types.destroy', $type->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce type ?');" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <div class="modal fade" id="editModal{{ $type->id }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <form method="POST" action="{{ route('decision-types.update', $type->id) }}">
                                @csrf
                                @method('PUT')
                                <div class="modal-content border-0 shadow">
                                    <div class="modal-header border-bottom-0 pb-0">
                                        <h5 class="modal-title fw-bold">Modifier le Type</h5>
                    ...
                    <tr>
                        <td colspan="4" class="text-center py-5 text-muted">
                            <i class="bi bi-file-earmark-text fs-1 d-block mb-3 text-light"></i>
                            Aucun type de décision configuré.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<div class="modal fade" id="createModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('decision-types.store') }}">
            @csrf
            <div class="modal-content border-0 shadow">
                <div class="modal-header border-bottom-0 pb-0">
                    <h5 class="modal-title fw-bold">Nouveau Type de Décision</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label fw-medium">Nom *</label>
                        <input type="text" name="name" class="form-control" required placeholder="Ex: Règlement, Directive, Recommandation...">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-medium">Code usuel (optionnel)</label>
                        <input type="text" name="code" class="form-control" placeholder="REG, DIR, REC...">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-medium">Description</label>
