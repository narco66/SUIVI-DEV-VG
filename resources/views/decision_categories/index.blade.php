            ...
            <div class="h-100 rounded-4 border bg-primary bg-opacity-10 p-4 shadow-sm">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div>
                        ...
                        <h2 class="h3 fw-bold text-dark mb-1">Catégories de Décision</h2>
                        <p class="text-muted mb-2">Organisez les décisions par catégorie thématique pour faciliter le pilotage et le reporting.</p>
                        <span class="badge bg-white text-primary border fw-semibold px-3 py-2">
                            {{ $categories->total() }} catégorie(s) configurée(s)
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="bg-light text-muted small text-uppercase">
                    <tr>
                        <th class="ps-4 py-3">Nom de la Catégorie</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="badge bg-secondary-subtle text-secondary">{{ $category->code ?: 'N/A' }}</span></td>
                        <td class="text-muted small">{{ \Illuminate\Support\Str::limit($category->description, 50) ?: '-' }}</td>
                        <td class="pe-4 text-end">
                            <div class="btn-group">
                                <button type="button" class="btn btn-sm btn-light border" data-bs-toggle="modal" data-bs-target="#editModal{{ $category->id }}" title="Modifier">
                                    <i class="bi bi-pencil text-primary"></i>
                                </button>
                                <form action="{{ route('decision-categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette catégorie ?');" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <div class="modal fade" id="editModal{{ $category->id }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog">
                            <form method="POST" action="{{ route('decision-categories.update', $category->id) }}">
                                @csrf
                                @method('PUT')
                    ...
                    <tr>
                        <td colspan="4" class="text-center py-5 text-muted">
                            <i class="bi bi-folder2-open fs-1 d-block mb-3 text-light"></i>
...
<div class="modal fade" id="createModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form method="POST" action="{{ route('decision-categories.store') }}">
            @csrf
            <div class="modal-content border-0 shadow">
                <div class="modal-header border-bottom-0 pb-0">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label fw-medium">Nom *</label>
                        <input type="text" name="name" class="form-control" required placeholder="Ex: Ressources Humaines, Finance...">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-medium">Code usuel (optionnel)</label>
                        <input type="text" name="code" class="form-control" placeholder="RH, FIN...">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-medium">Description</label>
